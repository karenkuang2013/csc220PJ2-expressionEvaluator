...
public class LispExpressionEvaluator 
{
	...
	private void evaluateCurrentOperation() 
	{
		...
		boolean num = true;
		while (num) {
			currentOperation = (String.valueOf(tokenStack.pop()));
			try 
			{
		...
		case "/":
			double den = operationStack.pop();
			result = 1 / den;
			if (!operationStack.empty()) 
			{
				result = ((den * den) * result);
				while (!operationStack.empty()) 
				{
					result /= operationStack.pop();
				}
	...
	public double evaluate() 
	{
        ...
        boolean ba = balance(currentExpression);
		...
		while (currentExpressionScanner.hasNext()) 
		{
			...
			if (currentExpressionScanner.hasNextInt()) 
			{
				...
				switch (item) {
				case '(':
					nextTok = currentExpressionScanner.next();
					nextItem = nextTok.charAt(0);

					if (nextItem == '*') 
					{
						tokenStack.push(nextItem);
					} 
					else if (nextItem == '/') 
					{
					...
					{
						throw new LispExpressionException("(  need to follow by an operator");
					}
					break;
				case ')':
					if(!ba)
					{
						throw new LispExpressionException("Not balance!");
					}
		...
		// Step 9: If you run out of tokens, the value on the top of tokenStack
		// is
		// is the result of the expression.
		//
		// return result
		// double result =
		// (Double.parseDouble(String.valueOf(tokenStack.pop())));
		// System.out.print("result1: "+result+"#");
		// double result;

		// result = (double) tokenStack.pop();
		return Double.parseDouble(String.valueOf(tokenStack.pop())); // return
																		// result
																		// here
		// return 0.0;
		// return result;
...
/**
 * This method is used to check balance of currentExpression
 * @param curExpr-- the input expression.
 * @return true when the parentheses are balanced and return false when they aren't balanced.
 */
private static boolean balance(String curExpr)
{
	MyStack<Character> check = new MyStack<Character>();
	int count = curExpr.length();
	boolean balance = true;
	int index = 0;
	char nextCh =' ';
	for(index = 0; balance && (index < count); index++)
	{
		nextCh = curExpr.charAt(index);
		if(nextCh == '(')
		{
			check.push(nextCh);
		}
		else if(nextCh == ')')
		{
			...
			{
				char openDeli = check.pop();
				balance = pair(openDeli, nextCh);
			}
		}
	}
	if(!check.empty())
...
/**
 * This private method is used to track the balance between open "(" and close ")"
 */
